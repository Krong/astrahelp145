...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("AstraHelp".to_owned());
    //mike
...
pub const RENDEZVOUS_SERVERS: &[&str] = &[ASTRA_SERVER];
pub const RS_PUB_KEY: &str = ASTRA_PUB_KEY;
//mike
...
impl Config2 {
	...
	//mike
	config.rendezvous_server = ASTRA_SERVER.to_string();
        config.options.insert("custom-rendezvous-server".to_string(), ASTRA_SERVER.to_string());
        config.options.insert("key".to_string(), ASTRA_PUB_KEY.to_string());
	config.options.insert("relay-server".to_string(), "".to_string());
	config.options.insert("api-server".to_string(), "".to_string());
	//mike
